...
(async () => {
  const { app } = await setup([
    { alias: 'star-pink-fill', src: getAssetUrl('star-pink-fill') },
  ]);
  ...
  const leftSub = createLabel(
    'shapes glide over one shared world-space grid',
    LEFT_CENTER.x,
    552,
    {
      size: 14,
  ...
  const rightSub = createLabel(
    'one tile per shape — setTransform() subdivides it',
    RIGHT_CENTER.x,
    552,
    {
      size: 14,
