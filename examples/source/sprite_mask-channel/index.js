...
import { setup, getAssetUrl } from './setup';
...
function createShowcase(centerX, centerY) {
  ...
  const title = createLabel('MASK', centerX, centerY - 8, {
    size: 34,
    fontWeight: '700',
  });
  const subTitle = createLabel('same mask sprite', centerX, centerY + 32, {
    size: 14,
    color: 0xa7c8de,
  });
...
(async () => {
  const { app, width } = await setup(
    SHOWCASE_ASSETS.map((name) => ({ alias: name, src: getAssetUrl(name) })),
  );
  ...
  maskGraphic
    .star(100, 100, 6, maskRadius * 0.75, maskRadius * 0.38)
    .fill(0xdd2200);

  const maskTexture = app.renderer.generateTexture({
    target: maskGraphic,
    resolution: 2,
  });
  ...
  previewBorder
    .circle(0, 0, maskRadius * 0.38 + 6)
    .stroke({ width: 1, color: 0x4a5c71, alpha: 0.6 });
  previewBorder.position.copyFrom(maskPreview.position);

  const previewLabel = createLabel('mask texture', centerX, showcaseY + 52, {
    size: 12,
    color: 0xcfd9e5,
  });

  const leftTitle = createLabel("channel: 'red'", leftCenterX, 206, {
    size: 20,
    color: 0xffb9c8,
  });
  const rightTitle = createLabel("channel: 'alpha'", rightCenterX, 206, {
    size: 20,
    color: 0xb1fff8,
  });
  const leftSub = createLabel(
    'red channel reveals the star',
    leftCenterX,
    530,
    { size: 14, color: 0xffcbd4 },
  );
  const rightSub = createLabel(
    'alpha channel reveals the full circle',
    rightCenterX,
    530,
    {
      size: 14,
  ...
  const sync = () => {
    ...
    leftMask.position.set(
      leftStartX + (leftEndX - leftStartX) * state.progress,
      showcaseY + yOffset,
    );
    rightMask.position.set(
      rightStartX + (rightEndX - rightStartX) * state.progress,
      showcaseY + yOffset,
    );
  };
