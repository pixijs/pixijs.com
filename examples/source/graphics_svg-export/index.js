...
(async () => {
  ...
  const leftLabel = createLabel('Graphics', leftCenterX, 160, {
    color: 0xffb9c8,
  });
  const rightLabel = createLabel('re-imported SVG', rightCenterX, 160, {
    color: 0xb1e8ff,
  });
  const arrow = createLabel('→', width / 2, badgeY, {
    size: 56,
    color: 0xffffff,
  });
  ...
  boxBg
    .roundRect(0, 0, boxWidth, boxHeight, 12)
    .fill({ color: 0x09111a, alpha: 0.82 });
  boxBg
    .roundRect(0, 0, boxWidth, boxHeight, 12)
    .stroke({ width: 2, color: 0x4fe6ff, alpha: 0.2 });
