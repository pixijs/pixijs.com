...
export default function MemberCard (props: MemberData): JSX.Element {
  ...
  for (const key in props.social) {
    links.push({ className: `header-link header-${key}-link`, to: props.social[key] })
  }
  ...
  return (
    <div className={`${styles.wrapper} col col--4`}>
      <div className={styles.card}>
        ...
        <h3 className='margin-bottom--md padding-horiz--md'>{props.name} {props.country}</h3>
        <div className={`${styles.badges} ${props.active ? '' : styles.inactive} margin-bottom--md`}>
          {props.badges.map((badge, i) => <h4 key={i}>{badge}</h4>)}
        </div>
