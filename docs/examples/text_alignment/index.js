...
// description: This example demonstrates improved text alignment parity, cycling through left, center, right, and justify alignment modes.
...
(async () => {
  ...
  const lineWidths = CanvasTextMetrics.measureText(SAMPLE_TEXT, metricsStyle).lineWidths.map((lineWidth) =>
    Math.min(lineWidth, boxWidth),
  );
  ...
  function getBarTarget(align, lineIdx) {
    ...
    const scaleX = align === 'justify' && lineIdx < numLines - 1 ? 1 : naturalScale;
  ...
  function queueAlignmentTransition(timeline, idx, holdBefore, fromAlign, fromBody, toBody) {
    ...
    timeline.to(leftGuide, { alpha: leftOn ? 0.66 : 0.12, duration: 0.3, ease: 'sine.out' }, switchPoint);
    timeline.to(rightGuide, { alpha: rightOn ? 0.56 : 0.12, duration: 0.3, ease: 'sine.out' }, switchPoint);
    ...
    if (isJustify) {
      ...
      timeline.to(flash, { alpha: 1, duration: 0.12, ease: 'sine.out' }, `${stepLabel}+=0.52`);
      timeline.to(flash, { alpha: 0, duration: 0.24, ease: 'sine.inOut' }, `${stepLabel}+=0.64`);
    }
  }
  ...
  introTl.to(leftGuide.scale, { y: textBlockH, duration: 0.55, ease: 'power2.out' }, 0.08);
  introTl.to(rightGuide.scale, { y: textBlockH, duration: 0.55, ease: 'power2.out' }, 0.16);

  introTl.to(label, { alpha: 1, y: labelY, duration: 0.45, ease: 'power2.out' }, 0.18);
  introTl.to(bodyA, { alpha: 1, x: boxX, y: bodyY, duration: 0.45, ease: 'power2.out' }, 0.24);
  ...
  introTl.to(activeDot.scale, { x: 1, y: 1, duration: 0.38, ease: 'back.out(2.1)' }, 0.35);
  introTl.to(leftGuide, { alpha: 0.66, duration: 0.34, ease: 'sine.out' }, 0.5);
  introTl.to(rightGuide, { alpha: 0.12, duration: 0.34, ease: 'sine.out' }, 0.5);
  ...
  function queueStep(idx, holdBefore) {
    queueAlignmentTransition(loopTl, idx, holdBefore, currentAlign, currentBody, nextBody);
