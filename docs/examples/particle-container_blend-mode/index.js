...
// description: This example demonstrates ParticleContainer inheriting blend modes from its ancestors, toggling an additive blend on the parent of the right-hand galaxy.
import { gsap } from 'gsap';
import { Container, Graphics, Particle, ParticleContainer, Text } from 'pixi.js';
...
function createGalaxy(texture, center) {
  ...
  const sync = (elapsed) => {
    for (const orbiter of orbiters) {
      ...
      const radius = orbiter.radius + Math.sin(elapsed * 1.4 + orbiter.wobblePhase) * 7;
...
(async () => {
  ...
  const leftChip = createLabel("parent.blendMode = 'normal'", LEFT_CENTER.x, 156, {
    color: 0xffd4ea,
    fontWeight: '700',
  });
  const rightChip = createLabel("parent.blendMode = 'normal'", RIGHT_CENTER.x, 156, {
    color: 0xffd4ea,
    fontWeight: '700',
  });
  const caption = createLabel('blend mode set on the parent — the ParticleContainer now inherits it', width / 2, 552, {
    size: 15,
    color: 0x9fb6cf,
  });

  app.stage.addChild(divider, leftGroup, rightGroup, leftChip, rightChip, caption);
  ...
  loop.to(rightChip.scale, { x: 1.12, y: 1.12, duration: 0.16, yoyo: true, repeat: 1 }, 1.5);
  loop.call(() => setRightMode('normal'), undefined, 4.5);
  loop.to(rightChip.scale, { x: 1.12, y: 1.12, duration: 0.16, yoyo: true, repeat: 1 }, 4.5);
  loop.to({}, { duration: 6 });
