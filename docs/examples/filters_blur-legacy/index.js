...
// description: This example compares the new optimized BlurFilter halving strength scheme against the legacy approach, showing improved visual quality at high blur strengths.
import { gsap } from 'gsap';
import { BlurFilter, Container, Sprite, Text } from 'pixi.js';
import { setup, getAssetUrl, ASSET_BASE } from '../shared/setup';
...
function createRadialPattern(cx, cy) {
  ...
  for (const ring of RINGS) {
    for (let i = 0; i < ring.count; i++) {
      ...
      sprite.position.set(cx + Math.cos(angle) * ring.radius, cy + Math.sin(angle) * ring.radius);
...
(async () => {
  ...
  container.addChild(leftShapes, rightShapes, divider, leftLabel, rightLabel, strengthText);
